...
export const LoginForm = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (res.data?.token) {
        setAuth(res.data.user || { email: data.email, name: data.email.split('@')[0] }, res.data.token);
        navigate('/workspaces');
      } else {
        setAuth({ email: data.email, name: data.email.split('@')[0] }, 'mock-jwt-token');
        navigate('/workspaces');
      }
    } catch (err) {
  ...
  return (
    <div className="w-full max-w-md p-8 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl space-y-6">
      <div className="text-center space-y-2">
        ...
        <p className="text-xs text-slate-400">Sign in to Collaborative Task Management</p>
      </div>
      ...
      <div className="text-center text-xs text-slate-400">
        Don't have an account? <Link to="/register" className="text-blue-400 font-semibold hover:underline">Create Account</Link>
      </div>
    </div>
  );
};