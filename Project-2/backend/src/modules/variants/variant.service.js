...
const createVariantType = async (productId, data, userId, userRole) => {
  ...
  if (exists) {
    ...
    error.status = 409; error.code = 'CONFLICT'; throw error;
  }
...
const getVariantTypesByProductId = async (productId) => {
  ...
  if (!product) {
    ...
    error.status = 404; error.code = 'NOT_FOUND'; throw error;
  }
...
const updateVariantType = async (productId, variantTypeId, data, userId, userRole) => {
  await verifyProductOwnership(userId, productId, userRole);
...
const deleteVariantType = async (productId, variantTypeId, userId, userRole) => {
  await verifyProductOwnership(userId, productId, userRole);
  const existing = await repository.findTypeById(variantTypeId);
  if (!existing) {
    const error = new Error('Variant type not found');
...
const createVariantOption = async (productId, variantTypeId, data, userId, userRole) => {
  await verifyProductOwnership(userId, productId, userRole);
  const type = await repository.findTypeById(variantTypeId);
  if (!type) {
    const error = new Error('Variant type not found');
...
const updateVariantOption = async (productId, variantTypeId, optionId, data, userId, userRole) => {
  await verifyProductOwnership(userId, productId, userRole);
  const type = await repository.findTypeById(variantTypeId);
  if (!type) {
    const error = new Error('Variant type not found');
...
const deleteVariantOption = async (productId, variantTypeId, optionId, userId, userRole) => {
  await verifyProductOwnership(userId, productId, userRole);
  const type = await repository.findTypeById(variantTypeId);
  if (!type) {
    const error = new Error('Variant type not found');
