...
describe("SKU Service", () => {
  ...
  describe("createSku", () => {
    ...
    it("should throw 404", async () => { productRepository.findById.mockResolvedValue(null); await expect(service.createSku("n", { skuCode: "X", variantOptionIds: [] })).rejects.toThrow("Product not found"); });
  ...
  describe("getSkusByProductId", () => {
    ...
    it("should throw 404", async () => { productRepository.findById.mockResolvedValue(null); await expect(service.getSkusByProductId("n")).rejects.toThrow("Product not found"); });
  });
